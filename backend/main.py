...
async def discover_network_clients(request: ICMPFloodRequest):
    ...
    try:
        attack = ICMPFloodAttack(interface=request.interface)
...
def get_active_attacks():
    ...
    try:
        ...
        for attack_id, attack in active_attacks.items():
            attack_type = "deauth" if "deauth" in attack_id else "icmp" if "icmp" in attack_id else "mitm" if "mitm" in attack_id else "unknown"
